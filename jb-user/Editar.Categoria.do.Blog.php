...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "cadpost")) {
  $updateSQL = sprintf("UPDATE blog_menu SET titulo=%s, autor=%s WHERE id=%s",
...
$colname_edtcatblog = "-1";
if (isset($_GET['id'])) {
  $colname_edtcatblog = $_GET['id'];
}
mysql_select_db($database_conn, $conn);
$query_edtcatblog = sprintf("SELECT * FROM blog_menu WHERE id = %s", GetSQLValueString($colname_edtcatblog, "int"));
$edtcatblog = mysql_query($query_edtcatblog, $conn) or die(mysql_error());
$row_edtcatblog = mysql_fetch_assoc($edtcatblog);
$totalRows_edtcatblog = mysql_num_rows($edtcatblog);
...
<div id="main">
  <div class="full_w">
    ...
    <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="cadpost" id="cadpost">
      <div class="element">
        <input name="id" type="hidden" id="id" value="<?php echo $row_edtcatblog['id']; ?>"/>
        <input name="username" type="hidden" id="username" value="<?php echo $row_userlogado['']; ?>"/>
        <label for="titulo">Titulo</label>
        <input name="titulo" class="text " id="titulo" value="<?php echo $row_edtcatblog['titulo']; ?>" />
      </div>
      <div class="entry">
